...
class AutoAttack():
    def __init__(self, model, norm='Linf', eps=.3, seed=None, verbose=True,
                 ...
                 device='cuda', log_path=None, num_classes=10):
        self.model = model
        self.norm = norm
        ...
        if not self.is_tf_model:
            ...
            # DEBUG: specify number of classes
            self.fab = FABAttack_PT(self.model, n_restarts=5, n_iter=100, eps=self.epsilon, seed=self.seed,
                                    norm=self.norm, verbose=False, device=self.device, n_target_classes=num_classes - 1)

            from .square import SquareAttack
            self.square = SquareAttack(
                self.model, p_init=.8, n_queries=5000, eps=self.epsilon, norm=self.norm, n_restarts=1, seed=self.seed,
            ...
            self.apgd_targeted = APGDAttack_targeted(
                self.model, n_restarts=1, n_iter=100, verbose=False, eps=self.epsilon, norm=self.norm, eot_iter=1,
                rho=.75, seed=self.seed, device=self.device, n_target_classes=num_classes - 1)

        else:
            from .autopgd_base import APGDAttack
            self.apgd = APGDAttack(
                self.model, n_restarts=5, n_iter=100, verbose=False, eps=self.epsilon, norm=self.norm, eot_iter=1,
                rho=.75, seed=self.seed, device=self.device, is_tf_model=True)
            ...
            from .square import SquareAttack
            self.square = SquareAttack(
                self.model.predict, p_init=.8, n_queries=5000, eps=self.epsilon, norm=self.norm, n_restarts=1,
                seed=self.seed, verbose=False, device=self.device, resc_schedule=False)
    ...
    def run_standard_evaluation(self, x_orig, y_orig, bs=250):
        ...
        with torch.no_grad():
            ...
            for batch_idx in range(n_batches):
                ...
                if SET_MASK:
                    # self.model.set_mask(torch.arange(start_idx, end_idx, device=x.device))
                    self.model.set_mask(x)
                # DEBUG
    ...
    def clean_accuracy(self, x_orig, y_orig, bs=250):
        ...
        for counter in range(n_batches):
            ...
            # DEBUG: set mask for IN-9 dataset experiment
            if SET_MASK:
                # idx = torch.arange(counter * bs, min((counter + 1) * bs, x_orig.shape[0]), device=x.device)
                # self.model.set_mask(idx)
                self.model.set_mask(x)
            # DEBUG
            # output = self.get_logits(x)
            # acc += (output.max(1)[1] == y).float().sum()
            output = get_pred(self.get_logits(x))
            acc += (output == y).float().sum()
    ...
    def set_version(self, version='standard'):
        ...
        if version in ('standard', 'standard-square+'):
            ...
            self.square.n_queries = 5000 if version == 'standard' else 10000

        elif version == 'non-diff':
