...
class APGDAttack:
    ...
    def dlr_loss(self, x, y):
        ...
        return -(
            x[u, y] - x_sorted[:, -2] * ind - x_sorted[:, -1] * (1.0 - ind)
    ...
    def attack_single_run(self, x, y, x_init=None, **kwargs):
        ...
        if self.norm == "Linf":
            ...
            x_adv = x + self.eps * torch.ones_like(x).detach() * self.normalize(
                t
            )
        elif self.norm == "L2":
        ...
        loss_best_steps = torch.zeros([self.n_iter + 1, x.shape[0]]).to(
            self.device
        )
        ...
        step_size = (
            alpha
            * self.eps
            * torch.ones([x.shape[0], *([1] * self.ndims)])
            .to(self.device)
            .detach()
        )
    ...
    def decr_eps_pgd(self, x, y, epss, iters, use_rs=True):
        ...
        for eps, niter in zip(epss, iters):
            ...
            x_init, acc, loss, x_adv = self.attack_single_run(
                x, y, x_init=x_init
            )
...
# NOTE: This function does not support binary classification
...
class APGDAttack_targeted(APGDAttack):
    ...
    @property
    def n_target_classes(self):
        return self._n_target_classes

    @n_target_classes.setter
    def n_target_classes(self, value):
        # DEBUG: prevent n_target_classes from exceeding the number of classes - 1
        self._n_target_classes = min(value, self._n_target_classes)
    ...
    def perturb(self, x, y=None, x_init=None, **kwargs_orig):
        ...
        for target_class in range(2, self._n_target_classes + 2):
            for counter in range(self.n_restarts):
                ind_to_fool = acc.nonzero().squeeze()
                if len(ind_to_fool.shape) == 0:
                    ind_to_fool = ind_to_fool.unsqueeze(0)
                if ind_to_fool.numel() != 0:
                    ...
                    # EDIT: x_to_fool is shortened here so mask has to be set again
                    if SET_MASK:
                        # self.model.set_mask(ind_to_fool)
                        self.model.set_mask(x_to_fool)
