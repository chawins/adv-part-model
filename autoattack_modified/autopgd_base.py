    ...
    def attack_single_run(self, x, y, x_init=None, **kwargs):
        ...
        if not self.is_tf_model:
            if self.loss == 'ce':
                ...
                def criterion_indiv(x, y): return -1. * F.cross_entropy(x, y, reduction='none')
            elif self.loss == 'dlr':
            ...
            if not self.is_tf_model:
                with torch.enable_grad():
                    ...
                    if logits.size(-1) > 1:
                        ...
                        loss_indiv = criterion_indiv(logits, y[:, None].float()).squeeze(1)
                    loss = loss_indiv.sum()
            ...
            with torch.no_grad():
                ...
                if self.norm == 'Linf':
                    ...
                    x_adv_1 = torch.clamp(torch.min(torch.max(
                        x_adv_1, x - self.eps), x + self.eps), 0.0, 1.0)
                    x_adv_1 = torch.clamp(torch.min(torch.max(
...
# DEBUG: This function does not support binary classification
