...
for i in {1..5}; do
    torchrun \
        ...
        --seg-const-trn 0.8 --semi-label 1 \
        --resume results/box1/checkpoint_best.pt --load-weight-only --resume-if-exist \
        --output-dir results/box15 --experiment part-pooling-4-semi && break
    sleep 30
done
...
python -u main.py \
    ...
    --output-dir results/box15 --experiment part-pooling-4-semi --evaluate
