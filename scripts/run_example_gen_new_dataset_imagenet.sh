...
# SEGPATH=$DATAPATH
# ============================== Part-ImageNet ============================== #
DATASET=part-imagenet-pseudo-imagenet-class
DATAPATH=/data/kornrapatp/PartImageNet
SEGPATH=$DATAPATH/PartSegmentations/All-imagenetclass-segtrain-processed/ # Path to train seg model (Check new_partition.py script)

OLD_DATASET=$DATAPATH/PartSegmentations/All-imagenetclass-segtrain-processed
NEW_DATASET=$DATAPATH/PartSegmentations/All-imagenetclass-suppressed-reweighted
NEW_SEGPATH=$NEW_DATASET-mask
JPEG_PATH=$DATAPATH/JPEGImages
NUM_NEW_SAMPLES=300000
PREDICTION_PATH=/data/kornrapatp/imagenetclass-predicted-suppressed-reweighted
PREDICTION_MODEL_PATH=/data/kornrapatp/results/ImagenetClass
...
# python3 pre_prediction_imagenet.py --old-dataset $OLD_DATASET --new-dataset $NEW_DATASET \
...
#     --dist-url tcp://localhost:$PORT --seed 0 --dist-backend $BACKEND \
#     --seg-backbone resnet101 --seg-arch deeplabv3plus --full-precision --pretrained \
#     --data $DATAPATH --seg-label-dir $NEW_SEGPATH --seg-labels 599 --dataset $DATASET --workers $NUM_WORKERS \
...
python3 post_prediction_imagenet.py --old-dataset $OLD_DATASET --new-dataset $NEW_DATASET \
    --jpeg-path $JPEG_PATH --num-new-samples $NUM_NEW_SAMPLES --prediction-path $PREDICTION_PATH
