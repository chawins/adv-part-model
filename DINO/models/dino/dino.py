        ...
        if hs_enc is not None:
            ...
            if hs_enc.shape[0] > 1:
                ...
                for layer_id, (
                    ...
                    layer_enc_outputs_coord = (
                        layer_enc_outputs_coord_unsig.sigmoid()
                    )
        ...
        if "aux_outputs" in outputs:
            for idx, aux_outputs in enumerate(outputs["aux_outputs"]):
                ...
                if (
                    self.training
                    and dn_meta
                    and "output_known_lbs_bboxes" in dn_meta
                ):
                    ...
                    l_dict["loss_xy_dn"] = torch.as_tensor(0.0).to("cuda")
                    l_dict["loss_hw_dn"] = torch.as_tensor(0.0).to("cuda")
                    l_dict["cardinality_error_dn"] = torch.as_tensor(0.0).to(
                        "cuda"
                    )
                    l_dict = {k + f"_{idx}": v for k, v in l_dict.items()}
                    losses.update(l_dict)
