"""Prepare PartImageNet dataset for object detection task."""
...
def main(split, label_dir, sample_proportion=0.10, bbox_discard_threshold=0.01):
    ...
    if args.use_imagenet_classes:
        if args.group_parts:
            os.makedirs(
                root / "image_labels" / "imagenet" / "grouped", exist_ok=True
            )
            os.makedirs(
                root / "annotations" / "imagenet" / "grouped", exist_ok=True
            )
            data_paths = {
                "train": (
                    root / "train",
            ...
            os.makedirs(
                root / "image_labels" / "imagenet" / "all", exist_ok=True
            )
            os.makedirs(
                root / "annotations" / "imagenet" / "all", exist_ok=True
            )
            data_paths = {
                "train": (
                    root / "train",
        ...
        if args.group_parts:
            os.makedirs(
                root / "image_labels" / "partimagenet" / "grouped",
                exist_ok=True,
            )
            os.makedirs(
                root / "annotations" / "partimagenet" / "grouped", exist_ok=True
            )
            data_paths = {
                "train": (
                    root / "train",
            ...
            os.makedirs(
                root / "image_labels" / "partimagenet" / "all", exist_ok=True
            )
            os.makedirs(
                root / "annotations" / "partimagenet" / "all", exist_ok=True
            )
            data_paths = {
                "train": (
                    root / "train",
    ...
    imagenet_id2name = {}
    with open("LOC_synset_mapping.txt", "r", encoding="utf-8") as file:
        for line in file:
            line = line.strip()
    ...
    for class_label, part_imagenet_class in enumerate(PART_IMAGENET_CLASSES):
        with open(
            f"{part_segmentations_path}/{part_imagenet_class}.txt",
            "r",
            encoding="utf-8",
        ) as file:
    ...
    for class_label, part_imagenet_class in enumerate(PART_IMAGENET_CLASSES):
        ...
        if "_sample" in split:
            seg_label_path = os.path.join(
                label_dir,
                f"PartSegmentations/All/{split.replace('_sample', '')}/{part_imagenet_class}/*.tif",
            )
        else:
            seg_label_path = os.path.join(
                label_dir,
                f"PartSegmentations/All/{split}/{part_imagenet_class}/*.tif",
            )
        ...
        if "_sample" in split:
            ...
            seg_filenames = seg_filenames[
                : int(sample_proportion * len(seg_filenames))
            ]
        ...
        for filename in seg_filenames:
            ...
            if args.group_parts:
                imarray = group_parts(
                    imarray, partimagenet_to_grouped_partimagenet
                )
            ...
            assert (
                len(image_labels) <= PART_IMAGENET_CLASSES[part_imagenet_class]
            )
            ...
            for _, part_label in enumerate(image_labels):
                ...
                lbl_0, _ = label(mask, return_num=True, connectivity=1)
                ...
                for _, prop in enumerate(props):
                    ...
                    small_bbox = (
                        bbox_to_image_area_ratio < bbox_discard_threshold
                    )
                    if small_bbox:
    ...
    # Remove last few images in validation set for debugging.
    # Otherwise distributed training does not work
    # TODO(chawins@): How many images are we dropping here?
    if "val" in split:
        images_json = images_json[:2400]

    # saving annotations
    json_object = json.dumps(image_to_label, indent=4)
    with open(data_paths[split][1], "w", encoding="utf-8") as outfile:
    ...
    with open(data_paths[split][2], "w", encoding="utf-8") as outfile:
        outfile.write(json_object)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="train",
        type=str,
        help="dataset split, one of train, val, test",
    )
    parser.add_argument(
        "--use-imagenet-classes",
        action="store_true",
        help="Use imagenet classes instead of part imagenet classes",
    )
    parser.add_argument(
        "--group-parts", action="store_true", help="Group part imagenet classes"
    )
    parser.add_argument(
        "--bbox-discard-threshold",
        default=0.01,
        help="bbox with bbox-to-image-area ratio smaller than this threshold will be discarded",
    )
    ...
    main(
        ...
        bbox_discard_threshold=args.bbox_discard_threshold,
    )
