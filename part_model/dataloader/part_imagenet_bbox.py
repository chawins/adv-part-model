...
def get_loader_sampler(args, transform, split: str):
    ...
    PART_IMAGENET_BBOX["part_to_class"] = part_imagenet_dataset.part_to_class
    PART_IMAGENET_BBOX["num_classes"] = part_imagenet_dataset.num_classes
    PART_IMAGENET_BBOX["num_seg_labels"] = part_imagenet_dataset.num_seg_labels
    ...
    setattr(args, "input_dim", PART_IMAGENET_BBOX["input_dim"])
    if is_train:
...
def load_part_imagenet(args):
    ...
    image_root, class_label_file, json_file = PATHS[split]
    ...
    name_train = 'part_imagenet_bbox_train'
    DatasetCatalog.register(name_train, lambda: load_coco_json(json_file, image_root, name_train))
    ...
    #     dataset=L(get_detection_dataset_dicts)(names=name_train),
    #     mapper=L(DatasetMapper)(
    #         # the defined two augmentations which will be random-selected during training.
    #         augmentations=[
    #             L(RandomResizedCrop)(size=img_size, return_params=use_atta),
    #             L(RandomHorizontalFlip)(flip_prob=0.5, return_params=use_atta),
    #         ],
    #         is_train=True,
    #         image_format = "RGB"
    #     ),
    #     total_batch_size=2,  # training batch size
    #     num_workers=4,
    ...
    #         # the defined two augmentations which will be random-selected during training.
    #         augmentation=[
    #             L(T.RandomCrop)(crop_type='absolute', crop_size=(img_size, img_size)),
    #             L(RandomResizedCrop)(size=img_size),
    #             # L(RandomHorizontalFlip)(flip_prob=0.5, return_params=use_atta),
    #         ],
    ...
    # # cfg = L(get_detection_dataset_dicts)(names=name_train)
    ...
    # return dataloader.train, None, None, None
    ...
    return dataloader, None, dataloader, dataloader
